...
class ParseNet(nn.Module):
  def __init__(self, vocab_size, label_size, embedding=50, hidden=200, inputs=48, l2=0.01, dropout=0.5):
    super(ParseNet, self).__init__()
    self.embedding = nn.Embedding(vocab_size, embedding)
    self.hidden = nn.Linear(embedding*inputs, hidden)
    self.output = nn.Linear(hidden, label_size)

  def forward(self, inputs):
    embeds = self.embedding(inputs)
    out = F.tanh(self.hidden(embeds))
    out = F.log_softmax(out)

    # TO BE CONTINUED, CHECKED, etc.
